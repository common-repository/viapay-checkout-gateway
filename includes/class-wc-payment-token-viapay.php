...
class WC_Payment_Token_Viapay extends WC_Payment_Token_CC
{
	/**
	 * Token Type String.
	 *
	...
	public function get_display_name( $deprecated = '' ) {
		ob_start();
		?>
		<img src="<?php echo esc_url( WC_HTTPS::force_https_url( WC()->plugin_url() . '/assets/images/icons/credit-cards/' . $this->get_card_type() . '.png' )); ?>" alt="<?php echo esc_html(wc_get_credit_card_type_label( $this->get_card_type() )); ?>" />
	...
	protected function get_hook_prefix() {
		return 'woocommerce_payment_token_viapay_get_';
	}
	...
	public function is_default() {
		...
		if ( WC_Gateway_Viapay_Checkout::wcs_is_payment_change() &&
	...
	public static function wc_get_account_saved_payment_methods_list_item( $item, $payment_token ) {
		if ( 'viapay' !== strtolower( $payment_token->get_type() ) ) {
			return $item;
		}
	...
	public static function wc_account_payment_methods_column_method( $method ) {
		if ( in_array( $method['method']['gateway'], WC_ViapayCheckout::PAYMENT_METHODS ) ) {
			$token = new WC_Payment_Token_Viapay( $method['method']['id'] );
	...
	public static function wc_get_saved_payment_method_option_html( $html, $token, $gateway ) {
		if ( in_array( $token->get_gateway_id(), WC_ViapayCheckout::PAYMENT_METHODS ) ) {
			// Revert esc_html()
...
add_filter( 'woocommerce_payment_methods_list_item', 'WC_Payment_Token_Viapay::wc_get_account_saved_payment_methods_list_item', 10, 2 );
add_action( 'woocommerce_account_payment_methods_column_method', 'WC_Payment_Token_Viapay::wc_account_payment_methods_column_method', 10, 1 );
add_filter( 'woocommerce_payment_gateway_get_saved_payment_method_option_html', 'WC_Payment_Token_Viapay::wc_get_saved_payment_method_option_html', 10, 3 );
