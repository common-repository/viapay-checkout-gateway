...
class WC_Viapay_Order_Statuses {
	/**
	 * Constructor
	 */
	public function __construct() {
		...
		define('VIAPAY_STATUS_CREATED', isset( $settings['status_created'] ) ? str_replace( 'wc-', '', $settings['status_created'] ) : 'pending' );
		define('VIAPAY_STATUS_AUTHORIZED', isset( $settings['status_authorized'] ) ? str_replace( 'wc-', '', $settings['status_authorized'] ) : 'on-hold' );
		define('VIAPAY_STATUS_SETTLED', isset( $settings['status_settled'] ) ? str_replace( 'wc-', '', $settings['status_settled'] ) : 'processing' );

		add_filter( 'woocommerce_settings_api_form_fields_viapay_checkout', array(
			$this,
			'form_fields'
		), 10, 2 );
		...
		add_filter( 'viapay_authorized_order_status', array(
			$this,
			'viapay_authorized_order_status'
		), 10, 2 );

		add_filter( 'viapay_settled_order_status', array(
			$this,
			'viapay_settled_order_status'
		), 10, 2 );
	...
	public function form_fields($form_fields) {
		...
		$form_fields['status_created'] = array(
			'title'       => __( 'Status: Viapay Created', 'viapay-checkout-gateway' ),
			'type'        => 'select',
		...
		$form_fields['status_authorized'] = array(
			'title'       => __( 'Status: Viapay Authorized', 'viapay-checkout-gateway' ),
			'type'        => 'select',
		...
		$form_fields['status_settled'] = array(
			'title'       => __( 'Status: Viapay Settled', 'viapay-checkout-gateway' ),
			'type'        => 'select',
	...
	public function woocommerce_create_order($order_id, $checkout)
	{
		...
		try {
			 ...
			 */
			if ( $order && $order->has_cart_hash( $cart_hash ) && $order->has_status( array( VIAPAY_STATUS_CREATED, 'failed' ) ) ) {
	...
	public function add_valid_order_statuses( $statuses, $order ) {
		if ( in_array( $order->get_payment_method(), WC_ViapayCheckout::PAYMENT_METHODS, true ) ) {
			$statuses = array_merge( $statuses, array( VIAPAY_STATUS_AUTHORIZED, VIAPAY_STATUS_SETTLED ) );
		}
	 ...
	 * @return string
	 */
	public function viapay_authorized_order_status( $status, $order ) {
		if ( in_array( $order->get_payment_method(), WC_ViapayCheckout::PAYMENT_METHODS, true ) ) {
			if ( WC_Payment_Gateway_Viapay::order_contains_subscription( $order ) ) {
				$status = 'on-hold';
			} else {
				$status = VIAPAY_STATUS_AUTHORIZED;
			}
		}
	 ...
	 * @return string
	 */
	public function viapay_settled_order_status( $status, $order ) {
		if ( in_array( $order->get_payment_method(), WC_ViapayCheckout::PAYMENT_METHODS, true ) ) {
			$status = VIAPAY_STATUS_SETTLED;
		}
	...
	public static function set_authorized_status( $order, $note = null, $transaction_id = null )
	{
		...
		if ( '1' === get_post_meta( $order->get_id(), '_viapay_state_authorized', true ) ) {
			return;
		}
		...
		self::update_order_status(
			$order,
			apply_filters( 'viapay_authorized_order_status', 'on-hold', $order ),
			$note,
			$transaction_id
		);

		update_post_meta( $order->get_id(), '_viapay_state_authorized', '1' );
	}
	...
	public static function set_settled_status( $order, $note = null, $transaction_id = null ) {
		...
		if ( ! in_array( $payment_method, WC_ViapayCheckout::PAYMENT_METHODS ) ) {
			return;
		}

		clean_post_cache( $order->get_id() );
		if ( '1' === get_post_meta( $order->get_id(), '_viapay_state_settled', true ) ) {
			return;
		}
		...
		if ( $invoice['settled_amount'] < $invoice['authorized_amount'] ) {
			...
			// @see WC_Viapay_Order_Statuses::payment_complete()
			// @see WC_Viapay_Order_Statuses::payment_complete_order_status()
			...
			update_post_meta( $order->get_id(), '_viapay_state_settled', '1' );
		}
	}
			...
			case 'cancelled':
				...
				if ( $gateway->can_cancel( $order ) ) {
					try {
						...
						// Rollback
						$order->update_status( $from, sprintf( __( 'Order status rollback. %s', 'viapay-checkout-gateway' ), $message ) );
					}
				}
				break;
			case VIAPAY_STATUS_SETTLED:
				// Capture payment
	...
	{
		if ( in_array( $subscription->get_payment_method(), WC_ViapayCheckout::PAYMENT_METHODS, true ) && $new_status === 'processing' ) {
			$can_be_updated = true;
		}
	...
	public function is_editable( $is_editable, $order ) {
		if ( in_array( $order->get_payment_method(), WC_ViapayCheckout::PAYMENT_METHODS, true ) ) {
			if ( in_array( $order->get_status(), array( VIAPAY_STATUS_CREATED, VIAPAY_STATUS_AUTHORIZED ) ) ) {
				$is_editable = true;
			}
		}
	...
	public function is_paid( $is_paid, $order ) {
		if ( in_array( $order->get_payment_method(), WC_ViapayCheckout::PAYMENT_METHODS, true ) ) {
			if ( in_array( $order->get_status(), array( VIAPAY_STATUS_SETTLED ) ) ) {
				$is_paid = true;
			}
		}
	...
	/**
	 * Prevent the pending cancellation for Viapay Orders if allowed
	 *
	...
	public function cancel_unpaid_order( $maybe_cancel, $order ) {
		if ( in_array( $order->get_payment_method(), WC_ViapayCheckout::PAYMENT_METHODS ) ) {
			$gateways = WC()->payment_gateways()->get_available_payment_gateways();
			$payment_method = $order->get_payment_method();
			if ( isset( $gateways[ $payment_method ] ) ) {
				/** @var WC_Gateway_Viapay $gateway */
				$gateway = $gateways[ $payment_method ];
